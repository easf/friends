...
def get_friends_for_connectedness(uidhash, mysql, token):
    ...
    cur.execute("SELECT * FROM (     SELECT "
    ...
    "ORDER BY `interactions_in_posts`.`post_owner` , SUM(`interactions_in_posts`.`total_interactions`) DESC ) AS interactions_in_posts_summary ", (uidhash,uidhash, uidhash))
    result = cur.fetchall()
    conn.commit()
...
def get_best_friends (uidhash, mysql):
    conn = mysql.connect()
    cur = conn.cursor()
    cur.execute("SELECT friend_fbid, friend_id, friend, total_interaction, po_fbid, post_owner  FROM (     SELECT "
        "`interactions_in_posts`.`po_fbid` AS `po_fbid`,"
        "`interactions_in_posts`.`po_id` AS `po_id`,"
        "`interactions_in_posts`.`post_owner` AS `post_owner`,"
        "`interactions_in_posts`.`friend_fbid` AS `friend_fbid`,"
    ...
    "GROUP BY `interactions_in_posts`.`po_fbid` , `interactions_in_posts`.`po_id` , `interactions_in_posts`.`post_owner` , `interactions_in_posts`.`friend_fbid` , `interactions_in_posts`.`friend` , `interactions_in_posts`.`friend_id`"
    "ORDER BY `interactions_in_posts`.`post_owner` , SUM(`interactions_in_posts`.`total_interactions`) DESC ) limit 10", (uidhash,uidhash,uidhash))
    result = cur.fetchall()
    top_ten = []
